...
namespace Finly.ViewModels
{
 ...
 private int _inactiveCategoriesCount; public int InactiveCategoriesCount { get=>_inactiveCategoriesCount; private set { _inactiveCategoriesCount=value; OnPropertyChanged(); } }
 private double _coveragePercent; public double CoveragePercent { get=>_coveragePercent; private set { _coveragePercent=value; OnPropertyChanged(); } }
 private int _totalTransactionsInPeriod; public int TotalTransactionsInPeriod { get=>_totalTransactionsInPeriod; private set { _totalTransactionsInPeriod=value; OnPropertyChanged(); } }
 private double _avgTransactionsPerActiveCategory; public double AvgTransactionsPerActiveCategory { get=>_avgTransactionsPerActiveCategory; private set { _avgTransactionsPerActiveCategory=value; OnPropertyChanged(); } }
 ...
 get
 {
 if (PreviousPeriodAmount ==0) return 0; // avoid div by zero; define as0 when previous is0
 ...
 private string GetStoredColor(string categoryName){ try { var id = DatabaseService.GetCategoryIdByName(UserId, categoryName); if(id.HasValue){ using var con = DatabaseService.GetConnection(); using var cmd = con.CreateCommand(); cmd.CommandText="SELECT Color FROM Categories WHERE Id=@id LIMIT 1;"; cmd.Parameters.AddWithValue("@id", id.Value); var obj = cmd.ExecuteScalar(); if(obj!=null && obj!=DBNull.Value) return obj.ToString()??string.Empty; } } catch { } return string.Empty; }
 ...
 TotalTransactionsInPeriod = list.Sum(x=>x.count);
 CoveragePercent = TotalCategoriesCount>0 ? ((double)ActiveCategoriesCount / (double)TotalCategoriesCount) *100.0 :0.0;
 AvgTransactionsPerActiveCategory = ActiveCategoriesCount>0 ? (double)TotalTransactionsInPeriod / (double)ActiveCategoriesCount :0.0;
 ...
 // Mark dominant categories by cumulative coverage threshold (e.g.,75%)
 double cumulative =0.0; foreach(var s in CategoryShares){ cumulative += s.Percent; s.Dominant = cumulative<=75.0; }
 }
 else{
 foreach(var it in list){ CategoryShares.Add(new CategoryShareItem{ Name=it.name, Amount=0m, Percent=0, Dominant=false}); }
 }
 ...
 TopCategories.Add(new CategoryShareItem{ Name=it.name, Amount=it.amount, Percent=pct, Dominant=false});
 }
 BottomCategories.Clear();
 foreach(var it in positive.OrderBy(x=>x.amount).Take(3)){
 double pct = totalAmount>0 ? (double)(it.amount/totalAmount)*100.0 :0.0;
 BottomCategories.Add(new CategoryShareItem{ Name=it.name, Amount=it.amount, Percent=pct, Dominant=false});
 }
 ...
 InactiveCategoriesCount = Math.Max(0, TotalCategoriesCount - ActiveCategoriesCount);
 TotalTransactionsInPeriod =0; CoveragePercent =0; AvgTransactionsPerActiveCategory =0;
 }
 }
 ...
 public class CategoryShareItem { public string Name { get; set; } = string.Empty; public double Percent { get; set; } public decimal Amount { get; set; } public bool Dominant { get; set; } }
 }
}
