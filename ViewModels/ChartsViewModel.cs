...
namespace Finly.ViewModels
{
    public class ChartsViewModel : INotifyPropertyChanged
    {
        ...
        {
            CategoriesSeries.Clear(); TrendSeries.Clear(); AccountsSeries.Clear(); WeekdaySeries.Clear();
            TrendLabels.Clear(); AccountLabels.Clear(); WeekdayLabels.Clear();
            TrendXAxes.Clear(); TrendYAxes.Clear(); AccountsXAxes.Clear(); AccountsYAxes.Clear(); WeekdayXAxes.Clear(); WeekdayYAxes.Clear();
            ...
            try
            {
                ...
                AccountsXAxes.Add(new Axis { Labels = AccountLabels.ToArray(), Name = "Konto / Ÿród³o", LabelsPaint = white, NamePaint = white });
        ...
        // BuildAggregates, BuildTrendList, GroupByWeekday, Safe* methods unchanged
        ...
        {
            // kopiowane z wczeœniejszej wersji – bez zmian
            if (SelectedMode == Mode.Cashflow)
            {
            ...
            // Expenses
            {
                ...
                string MapAcc(int? id) => id.HasValue ? (accounts.TryGetValue(id.Value, out var n) ? n : $"Konto {id.Value}") : "Gotówka / brak konta";
                var byAccount = rows.GroupBy(x => MapAcc(x.AccountId)).Select(g => (g.Key, g.Sum(x => Math.Abs(x.Amount))))
                    .OrderByDescending(x => x.Item2).ToList();
        ...
        private static decimal SafeDecimal(object? o) { if (o == null || o is DBNull) return 0m; if (o is decimal d) return d; if (o is double dbl) return (decimal)dbl; if (o is float fl) return (decimal)fl; return decimal.TryParse(Convert.ToString(o, CultureInfo.InvariantCulture), NumberStyles.Any, CultureInfo.InvariantCulture, out var parsed) ? parsed : 0m; }
