...
namespace Finly.ViewModels
{
 ...
 // Default: show all kinds true so initially everything visible
 ...
 public void DeleteTransaction(TransactionCardVm vm) { try { if (vm.Kind == TransactionKind.Expense) DatabaseService.DeleteExpense(vm.Id); else if (vm.Kind == TransactionKind.Income) DatabaseService.DeleteIncome(vm.Id); /* transfers could have delete if table supports */ } catch { } LoadFromDatabase(); }
 ...
 if (AllTransactions.Count ==0) return; // ensure data loaded first
 ...
 var q = (SearchQuery ?? string.Empty).Trim(); var cat = SelectedCategory?.ToString(); var acc = SelectedAccount?.ToString();
 bool anyTypeChecked = ShowExpenses || ShowIncomes || ShowTransfers; // if false -> show all types
 ...
 bool typeOk = !anyTypeChecked ||
 ((t.Kind == TransactionKind.Expense && ShowExpenses) ||
 (t.Kind == TransactionKind.Income && ShowIncomes) ||
 (t.Kind == TransactionKind.Transfer && ShowTransfers));
 bool catOk = string.IsNullOrEmpty(cat) || string.Equals(t.CategoryName, cat, StringComparison.CurrentCultureIgnoreCase);
 bool accOk = string.IsNullOrEmpty(acc) || string.Equals(t.AccountName, acc, StringComparison.CurrentCultureIgnoreCase);
 bool plannedOk = ShowScheduled || !t.IsPlanned;
 return textOk && dateOk && typeOk && catOk && accOk && plannedOk; }).ToList();
